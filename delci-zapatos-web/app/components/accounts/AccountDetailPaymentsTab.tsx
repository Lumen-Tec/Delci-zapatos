...
interface AccountDetailPaymentsTabProps {
  ...
  mode?: AccountDetailPaymentsTabMode;
...
export function AccountDetailPaymentsTab({
  ...
  const showHistoryPanel = mode === 'full' || mode === 'history';
  const showBiweeklyControls = mode === 'full' || mode === 'biweekly';
  const showRegisterControls = mode === 'full' || mode === 'register';
  const shouldShowNotifyButton = showNotifyButton && (mode === 'full' || mode === 'register');

  const layoutClassName = showControlsPanel && showHistoryPanel
    ? 'grid grid-cols-1 lg:grid-cols-2 gap-2 sm:gap-6 pb-6 md:pb-0'
    : 'pb-6 md:pb-0';
      ...
      {showControlsPanel && (
        <div className="bg-white rounded-xl sm:rounded-2xl border border-gray-100 shadow-sm sm:shadow-lg overflow-hidden">
          ...
          <div className="p-3 sm:p-6 space-y-2.5 sm:space-y-3">
            ...
            {showBiweeklyControls && (
              <>
                <InputField
                  label="Monto quincenal"
                  type="text"
                  size="sm"
                  value={formatAmountWithSpaces(biweeklyAmount)}
                  onChange={(value) => onBiweeklyAmountChange(normalizeAmountInput(value))}
                />
                        ...
                        {editingPaymentId === payment.id ? (
                          <div className="space-y-1.5 bg-rose-50/60 border border-rose-100 rounded-lg p-2">
                            ...
                            <div>
                              ...
                              <input
                                type="text"
                                value={formatAmountWithSpaces(editPaymentAmount)}
                                onChange={(e) => onEditPaymentAmountChange(normalizeAmountInput(e.target.value))}
                                className="w-full px-2 py-1 text-[11px] text-gray-900 bg-white border border-gray-300 rounded-md"
                              />
                            </div>
