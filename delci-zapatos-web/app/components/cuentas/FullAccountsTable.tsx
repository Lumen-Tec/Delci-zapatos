...
const TABS: { key: AccountTab; label: string }[] = [
  ...
  { key: 'proximos_pagos', label: 'Próximos pagos' },
];
...
export const FullAccountsTable = ({ accounts, onViewAccount, className = '' }: FullAccountsTableProps) => {
  ...
  const [activeTab, setActiveTab] = useState<AccountTab>('todas');
  ...
  const tabCounts = useMemo(() => {
    ...
    return {
      ...
      proximos_pagos: proximosCount,
    };
  }, [accounts]);

  const filteredAccounts = useMemo(() => {
    if (activeTab === 'proximos_pagos') {
      return accounts
        .filter((account) => {
          const isEligible =
  ...
  const renderProximosPagosTable = () => (
    <>
      <thead className="bg-gray-50 border-b border-gray-200 sticky top-0 z-10">
        <tr>
          <th className="px-2 sm:px-3 md:px-4 lg:px-6 py-2 sm:py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider whitespace-nowrap">
    ...
    <div className={`bg-white rounded-2xl shadow-lg border border-gray-100 ${className}`}>
      ...
      <div className="p-6 border-b border-gray-100">
        <div className="flex flex-col gap-4">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            ...
            <div className="grid grid-cols-2 sm:flex sm:items-center gap-1 p-1 bg-gray-100 rounded-xl w-full sm:w-auto">
              {TABS.map((tab) => (
                <button
                  key={tab.key}
        ...
        <div className="mt-4 flex flex-col gap-4">
          {activeTab === 'todas' ? (
            ...
            <div className="grid grid-cols-1 gap-3 max-w-md">
              {/* Cliente filter only */}
              <div>
                <label htmlFor="client-name-filter-proximos" className="block text-xs font-semibold text-gray-600 mb-1.5 uppercase tracking-wide">
                  Cliente
                </label>
                <div className="relative">
                  <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                  ...
                  <input
                    id="client-name-filter-proximos"
                    type="text"
                    value={filters.clientName}
                    onChange={(e) => handleFilterChange('clientName', e.target.value)}
                    placeholder="Buscar cliente..."
      ...
      <div className="overflow-x-auto">
        <table className="w-full">
          {activeTab === 'todas' && renderTodasTable()}
        ...
        <div className="flex flex-col gap-4">
          <div className="text-sm text-gray-600 text-center">
            ...
            <span className="font-medium text-gray-900">
              {activeTab === 'todas' ? accounts.length : tabCounts.proximos_pagos}
            </span>{' '}
            {activeTab === 'proximos_pagos' ? 'cuentas con pagos pendientes' : 'cuentas'}
          </div>
          {activeTab === 'todas' && (
